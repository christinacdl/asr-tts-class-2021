...
src_data = s5/data/train s5/data/dev s5/data/test 
...
# Data preparation 
# 1. Parts: Train vs Test vs Development datasets
train_set = $src_data/train.tsv
dev_set = $src_data/dev.tsv
test_set = $src_data/test.tsv
# 2. For each of these parts we need to have a separate subfolder with the following files in it:
# wav.scp text uutt2skp spk2utt
...
if [ $stage -le 1 ]; then
  ...
  for part in train dev test; do
    ...
    local/spanish_data_prep.sh $src_data ${part}_set data/$part
    local/spanish_data_prep.sh $src_data $src_data/$part.tsv data/$part
  done
  utils/combine_data/sh data/all data/train data/dev data/test
  
  utils/prepare_lm.sh --data data/train --locdata data/local/lm
  local/spanish_prepare_dict.sh data/local/lm data/local/dict_nosp
...
if [ $stage -le 7 ]; then
  ...
  local/spanish_format_lms.sh --src-dir data/lang data/local/lm
